...
module.exports = {
    ...
    create(context) {
        ...
        /**
         * Takes a MemberExpression, and attempts to find and mark a
         * related TSParameterProperty defined in the class's constructor.
         * @param {MemberExpression} node the member expression to search with.
         * @returns {void}
         * @private
         */
        function markTSParameterPropertyAsUsed(node) {
            if (node.object.type === "ThisExpression") {
                const scope = context.getScope();
                const propertyName = node.property.name;

                if (scope.upper && scope.upper.type === "class") {
                    const classScope = scope.upper;

                    if (!("TSParameterProperties" in classScope)) {
                        // create a convenience prop on the scope to save us doing this lookup multiple times
                        classScope.TSParameterProperties = {};
                        // iterate through the function scopes on the class to find the constructor
                        // use the child scopes instead of the class AST body, as the scopes exclude overload defs
                        classScope.childScopes.some(fnScope => {
                            const functionExpression = fnScope.block;
                            ...
                            if (
                                ...
                                functionExpression.parent.type ===
                                    "MethodDefinition" &&
                                functionExpression.parent.kind === "constructor"
                            ) {
                                // create a map of parameter name => function scope variables
                                classScope.TSParameterProperties = functionExpression.params.reduce(
                                    (acc, param, index) => {
                                        if (
                                            ...
                                            // ast-wise, multi-defining params is legal, so only use the first
                                            if (!acc[name]) {
                                                // arguments in index 0, shifting indexes by 1
                                                acc[name] =
                                                    fnScope.variables[
                                                        index + 1
                                                    ];
                                ...
                                // ast-wise, multiple constructor definitions are legal, so only use the first
                                return true;
                            }
                            return false;
                        });
                    ...
                    const variable =
                        classScope.TSParameterProperties[propertyName];
        ...
        return {
            ...
            MemberExpression: markTSParameterPropertyAsUsed,
